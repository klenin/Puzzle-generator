...
def Solve(puzzle):
    ...
    possibleValues = [[[],[],[],[],[],[],[],[],[]],
                      ...
                      [[],[],[],[],[],[],[],[],[]],   
                      [[],[],[],[],[],[],[],[],[]]]
    possibleValues = prepareToPossibleValues(possibleValues) #все варианты значений на каждую €чейку
    isSmhchanged = True
    while isSmhchanged:
        isSmhchanged = False
        for i in range(9):
            for j in range(9):
                if puzzleSolved[i][j]==0:
                    possibleValues = checkRow(i,j,puzzleSolved,possibleValues) #удалает лишние варианты заполнений €чейки из строки
                    possibleValues = checkColumn(i,j,puzzleSolved,possibleValues) #удалает лишние варианты заполнений €чейки из столбца
                    possibleValues = checkBlock(i,j,puzzleSolved,possibleValues) #удалает лишние варианты заполнений €чейки из квадрата
